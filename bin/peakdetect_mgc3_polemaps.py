...
for infilen in file_list:
  ...
  if 'r' in pmode: 
   ...
   if args.log: 
     ...
     c=m.contourf(xi1d,yi1d,np.log10(zii),clevels,cmap=colormap,vmin=args.vmin,vmax=args.vmax,antialiased=False)
   else:
     ...
     c=m.contourf(xi1d,yi1d,zii/10**lmax, clevels,cmap=colormap,vmin=args.vmin,vmax=args.vmax)
  ...
  if args.unsharp:
    ...
    nsm=np.int(args.nmed) #neighbourhood for median computation
    ...
    c1=ms.contourf(xi1d,yi1d,zi_smooth, clevels,cmap=colormap,vmin=0.)
    ...
    c3=mu.contourf(xi1d,yi1d,np.log10(zi_sharp),clevels,cmap=colormap,vmin=0.)
    ...
    c2=mn.contourf(xi1d,yi1d,(zi_sharp_cut), np.arange(0.,sigmax+1.,1.),**colormap_nsig)
  ...
  #convert fits to ndf
  try: 
    starlink_path=os.environ['STARLINK_DIR']
  except KeyError:
    sys.exit('\n Starlink installation not found, STARLINK_DIR variable not found. Please install Starlink and try again. \n Exiting...')
    
  starlink_path=starlink_path+'/bin'
